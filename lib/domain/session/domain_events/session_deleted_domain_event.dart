...
class SessionDeletedDomainEvent implements DomainEventInterface {

  final UuidValue _aggregateId;
  final UuidValue _eventId;
  final DateTime _occuredAt;
  final Session _session;
  static const String _eventName = "SessionDeletedDomainEvent";

  SessionDeletedDomainEvent._(this._aggregateId, this._eventId, this._occuredAt, this._session);
  ...
  static SessionDeletedDomainEvent fromSession(Session session) {
    String uuid = const Uuid().v4();
    UuidValue eventId = UuidValue.fromString(uuid);
    DateTime occuredAt = DateTime.now();
    return SessionDeletedDomainEvent._(session.id().value(), eventId, occuredAt, session);
  }
